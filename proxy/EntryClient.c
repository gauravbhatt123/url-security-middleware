 ...
 */
 ...
 #define TIMEOUT_SEC 5     // Socket recv/send timeout in secondsThe rapid growth of encrypted web traffic and the increasing sophistication of cyber threats have rendered traditional proxy-based security solutions less effective. This paper presents a robust, high-performance proxy server that integrates a hybrid deep learning model for real-time detection of malicious URLs in both HTTP and HTTPS traffic. The system combines a multithreaded C-based proxy with transparent TLS interception and a Python microservice running a CNN-LSTM classifier trained on the Kaggle “Malicious URLs Dataset.” The model achieves 97.45\% accuracy and responds in under 10 milliseconds per request, supporting over 800 requests per second with a cache hit ratio of 96\% on standard hardware. By analyzing only URLs, the system preserves user privacy while providing robust, real-time protection against phishing, malware, and defacement attacks. This work demonstrates that deep learning-based URL inspection can be effectively integrated into modern proxies, offering a scalable and explainable defense for encrypted and unencrypted web traffic.
 ...
 static void *handle_client(void *arg) {
     ...
     if (sscanf(buffer, "%15s %511s %15s", method, url, proto) == 3) {
             ...
             if (https_req_len > 0 && req_complete) {
                 ...
                 if (sscanf(https_req, "%15s %511s %15s", https_method, https_url, https_proto) == 3) {
                     ...
                     if (strcmp(https_method, "GET") == 0) {
                         ...
                         // Check cache for HTTPS request
                         pthread_mutex_lock(&cache_mutex);
                         CacheEntry *e = lookupcache(cache, req_host, path);
                         if (e) {
                             printf("[DEBUG] HTTPS Cache HIT\n");
                             printf("[DEBUG] Cached response size: %Lf\n", e->response_size);
                             printf("[DEBUG] First 200 chars of cached response: %.200s\n", e->response);
                             // Send cached response back to client
                             if (SSL_write(ssl_client, e->response, e->response_size) > 0) {
                                 printf("[DEBUG] Sent cached HTTPS response\n");
                             }
         ...
         // Correctly positioned 'else' block for plain HTTP requests
         } else {
